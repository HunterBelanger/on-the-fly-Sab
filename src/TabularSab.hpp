...
public:
  ...
  double operator()(double a, double b) const override final {
    ...
    if(a < alpha_.front() || a > alpha_.back() ||
       b < beta_.front() || b > beta_.back()) {
      return SCTSab(a, b);
    }
    ...
    if(interp == 1) {
      ...
      return ::LinearLogarithmic::apply(b, betaLow, betaHi, SaBetaLow, SaBetaHi);
    } else if(interp == 4) {
      return ::LogarithmicLinear::apply(b, betaLow, betaHi, SaBetaLow, SaBetaHi);
    } else if(interp == 5) {
      return ::LogarithmicLogarithmic::apply(b, betaLow, betaHi, SaBetaLow, SaBetaHi);
    } else {
      throw std::runtime_error("Invalid interpolation along beta in TabularSab");
    }
  ...
  double integrateAlpha(double aLow, double aHi, double b) const override final {
    ...
    if(b < beta_.front() || b > beta_.back()) {
      ...
      if(*betaIt == b) {
        ...
        if(interp == 1) {
          integral += ::Histogram::apply(b, betaLow, betaHi, SaBetaLow, SaBetaHi);
        } else if(interp == 2) {
          integral += ::LinearLinear::apply(b, betaLow, betaHi, SaBetaLow, SaBetaHi);
        } else if(interp == 3) {
          integral += ::LinearLogarithmic::apply(b, betaLow, betaHi, SaBetaLow, SaBetaHi);
        } else if(interp == 4) {
          integral += ::LogarithmicLinear::apply(b, betaLow, betaHi, SaBetaLow, SaBetaHi);
        } else if(interp == 5) {
          integral += ::LogarithmicLogarithmic::apply(b, betaLow, betaHi, SaBetaLow, SaBetaHi);
        } else {
          throw std::runtime_error("Invalid interpolation along beta in TabularSab");
        }
      }
