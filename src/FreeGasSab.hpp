...
class FreeGasSab : public Sab {
 ...
 private:
  ...
  double indefiniteIntegralAlphaExpBeta1(double aLow, double aHi,
                                         double b) const {
  ...
  double indefiniteIntegralAlphaExpBeta2(double a, double b) const {
    ...
    double T2 = (abs_b + b - 2.) *
                (std::exp(abs_b) * std::erf((a + abs_b) / (2. * std::sqrt(a))) -
                 std::exp(abs_b) + 1.);
    double C2 = a * std::erf((a + b) / (2. * std::sqrt(a)));
    double C3 = (2. * std::sqrt(a) * std::exp(-(a + b) * (a + b) / (4. * a))) /
                (std::sqrt(pi));
